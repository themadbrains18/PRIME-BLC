...
const FundingAccount = ({ showSideBar }) => {
  ...
  assets = assets.filter((item) => {
    return item.accountType === 'Funding Account';
  })
  let total = 0;
  assets.map((item) => {
  ...
  return (
    <div className="overview__container funding__account__container">
      ...
      <TransferComponent selectedToken={selectedToken} open={open} closeModel={closeModel} accountType="Funding Account" wallet="funding_wallet" />
    </div>
  )
}
