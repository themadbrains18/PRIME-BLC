...
import secondCoinList from '../../../../Core/secondCoinList.json'
import rupeeIcon from '../../../../assets/coinIcon/rupee.png'

const Buy = ({ active, setActive,show }) => {

  const [coinName, setcoinName] = useState('INR');
  const [recieveCoinName, setRecieveCoinName] = useState('USDT');
  const [paymentMethod, setPaymentMethod] = useState('');
  const [coinImage, setcoinImage] = useState(rupeeIcon);
  const [recieveCoinImage, setRecieveCoinImage] = useState('');
  ...
  const getInitTokenPrice = async () => {
    if (tokenList.length > 0) {
      let amount = await getTokenPrice(recieveCoinName, coinName)
      // console.log(amount)
  ...
  const selectCoin = async (data) => {
    ...
    let amount = await getTokenPrice(recieveCoinName, data.coinName)
    // console.log(amount)
    setConvertPrice(amount)
  }
  const selectReceiveCoin = async (data) => {
    ...
    let amount = await getTokenPrice(data.coinName, coinName)
    // console.log(amount)
    setConvertPrice(amount)
  }
  ...
  const submitData = (data) => {
    ...
    try {
      data.action = 0;
      data.token = coinName;
      data.recieveToken = recieveCoinName;
      data.paymentMethod = paymentMethod;
      navigate({
        pathname: '/p2p/trade/p2p',
        search: createSearchParams({
          type: data.action,
          token: data.recieveToken, ///receive token
          amount: data.spend,
          fiat: data.token, ///spend coin/currency
    ...
    <form onSubmit={handleSubmit(submitData)}>
      <div className='spend_wrapper'>
        <div className='input_wrapper'>
          ...
          <input className='spend_input' type="text" value={getValues('spend')} defaultValue="0" onChange={(e) => { calculateReceiveamount(e) }} name='spend' />
        </div>
        <div className='dropdown_wrapper' onClick={(e) => { activeDropdown(e, 1) }}>
          <div className="button_img">
            {coinImage === '' ?
              <svg width={12} height={12} viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
                <rect width={12} height={12} rx={24} fill="#FFA800" />
              ...
              </svg> : <img src={coinName === 'INR' ? rupeeIcon : coinImage} alt="" />}
          </div>
        ...
        {/* ====== list Dropdown ====== */}
        {active == 1 && <CoinListDropdown tokenList={firstCoinList} selectCoin={selectCoin} />
        }
      </div>
      {/* {errors.spend?.message && <p className='errorMessage validation_msg'>{errors.spend?.message}</p>} */}
      <p className='validation_msg'>{errors.spend?.message ? errors.spend?.message : ''}</p>

      <div className='spend_wrapper'>
        <div className='input_wrapper'>
          <label className='input_label'>Receive ≈</label>
          <input className='spend_input' type="text" value={receivedAmount} defaultValue="0" name='receive' {...register('receive')}></input>
        </div>
        <div className='dropdown_wrapper' onClick={(e) => { activeDropdown(e, 2) }}>
          <div className="button_img">
            {recieveCoinImage ? <img src={recieveCoinImage} alt="" /> : <svg width={48} height={48} viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        {active == 2 && <ReceiveCoinListDropDown tokenList={tokenList} selectCoin={selectReceiveCoin} />}
      </div>
      <p className='validation_msg recive'>1 {recieveCoinName} ≈ {convertPrice} {coinName}</p>

      <div className='payment_method_wrapper'>
        <p className='payment_method_heading'>
          Payment Methods
        </p>
        <div className='payment_inner' onClick={(e) => { activeDropdown(e, 5) }}>
        ...
        {active == 5 && <AllPaymentMethods selectPayment={selectPayment} />}
      </div>
      ...
      : <button type='submit' className='submit_cta'>Buy with {coinName}</button> }
      </div>
    </form>
  )
}
